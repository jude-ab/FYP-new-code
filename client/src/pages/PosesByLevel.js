    ...
    const fetchPoses = async () => {
      try {
        const response = await axios.get(
          `https://yogahub-backend-7cd139cc022f.herokuapp.com/api/yoga/poses?level=${(level)}`
        );
          ...
          <ModalContent borderRadius="25px" boxShadow="0 2px 10px rgba(0, 0, 0, 0.5)">
            ...
            <ModalBody fontFamily="Work sans">
              ...
              {selectedPose?.ImagePath && (
                  <img src={`https://yogahub-backend-7cd139cc022f.herokuapp.com/${selectedPose.ImagePath}`} alt={selectedPose.AName} style={{ width: '100%', marginTop: '10px' }} />
              )}
          </ModalBody>
          </ModalContent>
        </Modal>
