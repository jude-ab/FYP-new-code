  ...
  datasets: [
    {
      ...
      data: moods.happy.map(mood => {
        ...
        const dayMonthLabel = `${date.getDate()} ${date.toLocaleString('default', { month: 'short' })}`;
        if (currentWeekLabels.includes(dayMonthLabel)) {
          return { x: dayMonthLabel, y: mood.value };
        }
        return null;
